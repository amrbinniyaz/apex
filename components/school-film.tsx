...
export function SchoolFilm({
  ...
  useEffect(() => {
    ...
    manuallyPaused.current = window.matchMedia(
  ...
  function changePopup(open: boolean) {
    ...
    else if (inView.current && !document.hidden && !manuallyPaused.current)
      player.current?.playVideo();
  }
  ...
  return (
    <>
      <div className="school-film-window">
        <Image
          className={`school-film-poster ${hasStarted && !failed ? 'is-playing' : ''}`}
      ...
      <Dialog open={filmOpen} onOpenChange={changePopup}>
        <DialogTrigger
          className="school-film-full"
          aria-label="Play full school video"
        >
          <span>
        ...
        <DialogContent className="school-film-dialog" initialFocus={filmTitle}>
          <DialogTitle ref={filmTitle} tabIndex={-1}>
            Discover Apex International School
          </DialogTitle>
          {filmOpen && <FullSchoolFilm />}
          <a
            href="https://www.youtube.com/watch?v=s3l7IsnY_jk&t=6s"
            target="_blank"
        ...
        {!failed && (
          <>
            <button
              ...
              disabled={!ready}
              aria-label={playing ? 'Pause school film' : 'Play school film'}
            >
              {playing ? <Pause size={16} /> : <Play size={16} />}
            </button>
            <button
              className="film-audio-toggle"
              onClick={toggleAudio}
              disabled={!ready}
              aria-label={muted ? 'Unmute school film' : 'Mute school film'}
            >
              {muted ? <VolumeX size={18} /> : <Volume2 size={18} />}
              <span className="film-control-label">
...
function FullSchoolFilm() {
  ...
  return (
    <div ref={stage} className="full-film-stage">
      <div ref={host} className="full-film-mount" />
      {failed ? (
        <p className="full-film-error">
          The film couldn’t load. Please use the YouTube link below.
        </p>
      ) : (
        <div className="full-film-controls">
          <button
            disabled={!ready}
            aria-label={playing ? 'Pause full film' : 'Play full film'}
            onClick={() =>
              playing
                ? player.current?.pauseVideo()
                : player.current?.playVideo()
          ...
          <button
            disabled={!ready}
            aria-label={muted ? 'Unmute full film' : 'Mute full film'}
            onClick={() => {
              if (muted) player.current?.unMute();
              else player.current?.mute();
              setMuted(!muted);
          ...
          <input
            type="range"
            aria-label="Seek full film"
            aria-valuetext={`${timestamp(elapsed)} of ${timestamp(duration)}`}
            min="0"
            max={duration || 1}
            step="1"
          ...
          />
          <span className="full-film-time">
            {timestamp(elapsed)} / {timestamp(duration)}
          </span>
          <button
            aria-label="Fullscreen film"
            onClick={() => {
